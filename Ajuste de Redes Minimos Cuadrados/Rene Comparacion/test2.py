...
def crear_vector_gradiente(puntos, observaciones, indices_diccionario, vector_parametros):
    A = []
    W = []
    for obs in observaciones:
        ...
        row = np.zeros_like(vector_parametros)
        for i in range(3):
            row[idx_a + i] = partial_pt1[i]
            row[idx_b + i] = partial_pt2[i]
        
        row[idx_obs] = -1  # -1 en el índice de la observación
        
        A.append(row)
        W.append(w)
    
    return np.array(A), np.array(W)

A, W = crear_vector_gradiente(puntos, observaciones_ordenadas, indices_diccionario, vector_parametros)
